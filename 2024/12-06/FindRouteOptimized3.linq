...
void Main()
{
	...
	var loopingRoutes = 0;
	Parallel.For(1, uniquePositions.Count, i => // Start from 1 to skip the first position
	{
		var p = uniquePositions[i];
		var newMap = (char[,])map.Clone();
		newMap[p.R, p.C] = '#';
		var newGuard = new Guard(map);
		newGuard.GetRoute();
		if (newGuard.IsRouteLoop)
		{
			Interlocked.Increment(ref loopingRoutes); // Thread-safe increment
		}
	});
...
public class Guard
{
	...
	public List<Position> GetRoute()
	{
		...
		{
			if (!uniqueRouteSteps.Add(currentStep)) // Combine Contains and Add
			{
				IsRouteLoop = true;
				break;
			}
